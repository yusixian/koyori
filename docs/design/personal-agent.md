...
> 2026-09-26；本地自配模型文字 Agent 与单 Skill 按需讨论已进入开发版；原讨论切片的交付证据见 [PR #5](https://github.com/yusixian/koyori/pull/5)。#30 分支新增本机偏好查看、管理入口与撤销预览，已有本机测试但尚非公开 Preview 的发布证据。#28 开发分支新增从摘要预览返回使用证据的入口，同样尚非公开 Preview 的发布证据。个人 Bot 远端接入、通用工具操作卡和语音仍是候选阶段。本文不复制 Bot 代码、修改服务器或部署任何服务。
...
统计摘要使用生成时的快照，不随账本自动变化；未知、同名待归属和暂停状态仍需解释。摘要预览可返回所选 Skill 的使用账本证据区，沿用同一观察窗口；没有证据或来源不可用时给出提示，返回不会改写草稿或自动发送。当前不支持多选或全文附件，摘要正文仍不含证据文件位置；整理建议没有操作卡和执行权限。

本地操作卡现支持选定 Skill 的“始终保留”和“30 天后复查”：由用户点选动作，宿主生成当前偏好与结果预览，确认时重新核对资源、来源和偏好，五分钟过期。模型文字不会生成执行参数或授权。项目级受管部署和撤销由 Skills 管理界面单独计划与确认。

Agent 选定 Skill 时，偏好区域应读取本机使用账本，并清晰标出当前偏好（始终保留、复查日期或无标记）、对应账本来源及记录状态；不把缺失或不可用来源伪装成无偏好。提供前往既有“使用与建议”入口查看证据的链接，不复制或自动发送账本记录。撤销标记也使用预览—确认操作卡：预览说明将清除哪项偏好；取消不写入，确认后清除偏好但保留首次观察日期。卡片需过期，并在主进程保存前再次核对目标 Skill 资源、来源/偏好版本；资源或偏好已变化时拒绝旧预览并要求重新生成。上述变更只作用于本机使用账本，不修改 Skill 文件，不授予模型写入权限。

候选远端阶段才讨论个人 Bot 的设备配对、远端会话、获准能力、操作卡和语音。它们不属于当前本地文字或按需摘要阶段，也不代表已有可调用 endpoint。

创新优先放在三处可体验的联系：

- **对话跟着工作走**：从资源详情进入讨论；预览可返回所选 Skill 的使用证据区核对，摘要正文只保留名称、观察窗口和统计缺口，不会后台读取所有项目。
- **记忆看得见**：本地账本偏好在 Agent 中显示来源和状态，用户可通过预览确认设置或撤销；区分当前对话、本机使用偏好与 Bot 远端记忆。不会默认合并 Telegram 群聊历史。
...
- 未知、同名待归属和暂停状态保留解释；预览可返回所选 Skill 的使用证据区，缺少记录或来源时明确提示。当前不支持多选或全文附件，摘要正文不附加证据文件位置，建议不授予操作卡或执行权限。
...
本地文字阶段的最小会话能力为创建/继续/重命名/删除，保存消息与状态、停止生成、明确错误重试。选定 Skill 后可展示本机账本中的明确偏好与来源/状态，链接到已有使用证据区，并通过用户确认的操作卡设置或撤销偏好；撤销保留首次观察日期，取消预览无写入，保存前由主进程重新核对当前资源与偏好。没有会话树、多 Agent 调度、终端、任意 MCP 安装或常驻自主任务。单 Skill 讨论复用同一会话与连接边界。
...
7. 偏好切片需验证选定 Skill 时正确显示始终保留/复查日期、账本来源和状态；去往既有证据入口；设置与撤销卡片的确认/取消；撤销后首次观察日期不变；以及 Skill 资源或偏好在预览后变化时主进程拒绝保存。当前验证结果待记录，不能用 mock 替代真实桌面流程。
8. 外部条件未具备时保留未完成项，或请用户审阅调整里程碑；不能用 mock 替代真实首版承诺。
