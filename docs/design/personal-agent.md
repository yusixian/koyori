...
> 2026-09-22；整体方向已确认，具体 Agent 接口将在后续里程碑实现。本文没有开放接口、复制 Bot 代码、修改服务器或部署任何服务。语音深度仍待用户选择。

## 1. 用户想要的体验

Koyori 有一个可持续交流的个人 Agent：有稳定的角色与表达方式，理解用户主动交给它的工作台上下文，能解释 Skills 整理建议，并把建议变成可审阅的操作卡。它也是 cos-tool-bot 在桌面上的新入口。

一个具体场景：在 Skills 页选择几项资源，点“和 Koyori 一起整理”。对话附上可预览的内容和使用摘要；Agent 解释重复或低频的可能原因，允许用户纠正，再生成“保留 / 稍后复查 / 撤销受管部署”卡。点击卡后由本机检查最新状态、展示影响并执行，Agent 不能用聊天文字代替授权。

创新优先放在三处可体验的联系：

- **对话跟着工作走**：从资源详情进入讨论，答案引用具体资源和观察窗口，点击引用可回到原处；不会后台读取所有项目。
...
首版最小会话能力为创建/继续/重命名/删除，保存消息与状态、停止生成、明确错误重试。没有会话树、多 Agent 调度、终端、任意 MCP 安装或常驻自主任务。

公开用户应能自配一个经验证的模型协议使用基础 Agent；个人连接 cos-tool-bot 后启用服务器能力。两种连接共享界面契约，不维护两套完整产品：基础模式只做文字、明确偏好和少量本地查询/提案；不复制 Bot 的工具全集。切换连接新建会话，提示数据去向，不静默迁移历史或在错误时自动切服务。
...
1. 先用合成后端验证原生对话、流式回复、取消和故障；这不算真实 Bot 接入完成。
2. 在 Bot 的隔离开发环境跑通配对、独立桌面会话、至少一项获准的服务器能力；无意外 Telegram 发送，不扩大现有后台权限。
3. 验证跨用户/跨会话拒绝、设备撤销、重复发送、断线续流、取消、未知用量和私有字段过滤。合成 payload 用明显假数据。
4. 跑通“选定 Skills → Agent 解释 → 生成操作卡 → 本地确认与 revision 检查 → 撤销/恢复”。模型仅生成文字建议不算操作集成完成。
5. 语音若纳入，补播放顺序、停止后旧音频不续播、服务错误和选定目标设备实测。语音许可/算力未具备时不能把该项勾为完成。
6. 外部条件未具备时保留未完成项，或请用户审阅调整里程碑；不能用 mock 替代真实首版承诺。
